---
title: Get Webhook
description: Retrieve a single webhook configuration by webhookId
sidebar:
  order: 3
---

import { Tabs, TabItem } from '@astrojs/starlight/components';

`GET https://api.echovalue.dev/webhook/<webhookId>`

Returns a single webhook configuration. Responses differ slightly for webmail-triggered and cron-triggered webhooks.

:::note
**Cost:** 1 credit
:::

<Tabs syncKey="lang">
<TabItem label="cURL">

```sh
curl 'https://api.echovalue.dev/webhook/slack' \
  -H 'x-token: mytoken'
```

</TabItem>
<TabItem label="JavaScript">

```js
fetch('https://api.echovalue.dev/webhook/slack', {
  headers: { 'x-token': 'mytoken' }
})
.then(response => response.json())
.then(data => console.log(data));
```

</TabItem>
<TabItem label="Python">

```python
import requests

response = requests.get('https://api.echovalue.dev/webhook/slack',
  headers={'x-token': 'mytoken'}
)
print(response.json())
```

</TabItem>
<TabItem label="PHP">

```php
<?php
$ch = curl_init('https://api.echovalue.dev/webhook/slack');
curl_setopt($ch, CURLOPT_HTTPHEADER, ['x-token: mytoken']);
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
$response = curl_exec($ch);
curl_close($ch);
print_r(json_decode($response));
?>
```

</TabItem>
<TabItem label="Go">

```go
package main

import (
    "fmt"
    "io"
    "net/http"
)

func main() {
    req, _ := http.NewRequest("GET", "https://api.echovalue.dev/webhook/slack", nil)
    req.Header.Set("x-token", "mytoken")

    client := &http.Client{}
    resp, _ := client.Do(req)
    defer resp.Body.Close()
    body, _ := io.ReadAll(resp.Body)
    fmt.Println(string(body))
}
```

</TabItem>
</Tabs>

## Single Webhook Response

`200 OK` — JSON object.

```json
{
  "webhookId": "daily-sync",
  "webhook": "https://example.com/hooks/daily-sync",
  "headers": {},
  "format": "",
  "options": {},
  "schedule": {
    "cron": "0 9 * * *",
    "timezone": "Europe/Rome",
    "active": true,
    "nextRunAt": "2026-04-24T07:00:00.000Z"
  },
  "hash": "a1b2c3d4e5f6..."
}
```

| Field | Type | Description |
| ----- | ---- | ----------- |
| `webhookId` | string | Public webhook identifier used for update/delete/test operations |
| `webhook` | string | Your configured callback URL |
| `headers` | object | Custom headers (values are redacted) |
| `format` | string | Configured native format, if any (e.g. `slack`, `telegram`, `custom`) |
| `template` | object | Custom template object (only present when format is `custom`) |
| `options` | object | Format-specific options (sensitive values like `routing_key` are redacted as `***`) |
| `includeAttachments` | boolean | Whether attachments are included in webhook payload. Returned only for webmail-triggered webhooks. |
| `schedule` | object | Cron schedule configuration and runtime status for scheduled webhooks |
| `email` | string | Opaque mailbox address that triggers this webhook. Returned only for webmail-triggered webhooks. |
| `hash` | string | SHA256 hash of your token — use this to identify incoming payloads |

**Response headers:**

| Header | Description |
| ------ | ----------- |
| `x-balance` | Wallet balance after this call |
| `x-cost` | Credits consumed |

See [Response Headers](/response-headers/) for details.

| Status | Meaning |
| ------ | ------- |
| `200` | Webhook returned |
| `400` | Invalid request |
| `401` | Invalid token |
| `402` | Insufficient credits |
| `404` | Webhook not found |
