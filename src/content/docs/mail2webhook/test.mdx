...
:::note
For webmail-triggered webhooks, the test sends a dummy email-style payload. For cron-triggered webhooks, the test verifies the configured target URL and returns delivery status, but it does not relax the 30-day scheduling limit.
:::
