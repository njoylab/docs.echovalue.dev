...
These headers appear on wallet-consuming endpoints such as key-value operations, Mail2Webhook management endpoints, token balance/settings endpoints, and other endpoints that explicitly emit them in the OpenAPI file. `GET /token/logs` remains free, but the current OpenAPI file declares `x-cost` on its `200` response.
