...
These headers appear on wallet-consuming endpoints such as key-value operations, Mail2Webhook management endpoints, and token balance/settings endpoints. `GET /token/logs` is free and does not return wallet headers.
