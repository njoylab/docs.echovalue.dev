...
:::note
**Cost:** 1 credit. For ongoing balance monitoring, use the `x-balance` response header on paid endpoints instead of calling this endpoint separately.
:::
...
Configure automatic notifications when your wallet balance falls below a threshold. Notifications are sent at most once per low-balance incident — the cooldown resets when your balance rises back above the threshold.
...
| `options` | object | Format-specific options (e.g. `chat_id` for Telegram, `routing_key` for PagerDuty) | No |
