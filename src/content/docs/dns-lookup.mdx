...
| `spf_strict` | boolean | Whether SPF enforcement is strict |
| `dmarc_policy` | string | DMARC policy: `none`, `quarantine`, or `reject` |
| `has_dkim` | boolean | Whether DKIM was detected |
| `email_security_score` | number | Enrichment score from `0` to `100` |
| `ssl_certificate_expires_at` | string (ISO 8601) | TLS certificate expiration time when SSL inspection runs |
| `ssl_days_until_expiry` | integer or null | Days left before certificate expiry |
| `warnings` | string[] | Warnings produced by enrichment or SSL analysis |
| `recommendations` | string[] | Actionable follow-up items |
| `enrichment` | object | Additional upstream enrichment payload when available |
| `metadata` | object | Lookup metadata such as query time and record count |
| `errors` | object | Per-record-type lookup errors when some queries fail |
...
The response is now partially typed in the OpenAPI spec, but some nested enrichment data remains upstream-defined and can still vary by target and enabled options.
