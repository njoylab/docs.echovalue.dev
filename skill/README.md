# EchoValue API Skill

This directory contains the Claude skill and the canonical reference files for EchoValue.

## Install

Use [`INSTALL.md`](../INSTALL.md) for the full install flow, including Codex, ChatGPT, Cursor, and Continue.

## Files

- [`SKILL.md`](./SKILL.md) - Claude skill instructions
- [`quick-reference.md`](./quick-reference.md) - API reference and examples
- [`install-skill.sh`](../install-skill.sh) - Installer

## Docs

- [EchoValue Documentation](https://docs.echovalue.dev)
- [LLM-optimized docs](https://docs.echovalue.dev/llms-small.txt)
- [OpenAPI Spec](https://docs.echovalue.dev/openapi.yaml)
