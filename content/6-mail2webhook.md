...
------- | -----------
x-token | Your wallet ID (minimum 10 characters)
x-mailbox-secret | Shared secret (matched against MAILBOX_SECRET env)

### Request Body
```json
{
  "to": { "hash": "target-wallet-hash" },
  "payload": { "your": "data" }
}
```
...
-H 'Content-Type: application/json' \
-d '{"to":{"hash":"a1b2c3d4e5f6..."},"payload":{"event":"test"}}'
```
...
<aside class="notice">
Costs: 1 credit (deducted from caller's wallet)
</aside>

<aside class="warning">
Security: Always use the hash instead of the wallet email in the `to` field to avoid exposing tokens.
</aside>